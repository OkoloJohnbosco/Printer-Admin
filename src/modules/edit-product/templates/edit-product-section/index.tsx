...
import useUpdateProduct from "@/lib/hooks/admin/use-update-product";
...
export default function EditProductSection({
    ...
    defaultValues: {
      ...
      specifications:
        Object.keys(product.specifications || {}).length > 0
          ...
          : [{ key: "", label: "", options: [""], base: "", priceType: "" }],
  ...
  useEffect(() => {
    if (product) {
      ...
      const addonEntries = (product.addons || []).map((addon) => ({
        key: addon.key || addon.label,
        label: addon.label,
        description: addon.description || "",
      }));
  ...
  const onSubmit = (data: FormData) => {
    ...
    updateProduct.mutateAsync(productData).then(() => {
        ...
        <div className="mb-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm sm:text-base">
              Update the product specifications and settings
            </p>
          </div>
        </div>
